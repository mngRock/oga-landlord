// This is a simple, server-safe component for the landlord's view.

export default function LandlordPropertyDetailView({ property }: { property: any }) {
  const getFullAddress = () => {
    const p = property.buildings || property;
    return `${p.address}, ${p.lga || ''}, ${p.city}, ${p.state}`.replace(/ ,/g, ',');
  };

  const mediaItems: { url: string; type: string; }[] = (property.media_urls || []).map((item: any) => 
    ...
    <>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mb-6">
        {mediaItems.length > 0 ? mediaItems.slice(0, 4).map((item, index) => (
          <div key={index} className={`relative block group rounded-lg overflow-hidden ${index === 0 ? 'col-span-2 row-span-2' : 'h-40'}`}>
            <img 
